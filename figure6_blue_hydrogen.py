...
Manuscript figure: charts/gasfield_sink_combined_gate_map.png
"""
...
def plot_cost_supply_curve(hubs_df: pd.DataFrame, output_path: str):
...
def generate_blue_hydrogen_gate_maps(*, reuse_assignments: bool = False):
    ...
    if reuse_assignments and gasfield_csv.exists() and sink_csv.exists():
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--reuse-assignments",
        action="store_true",
        help="Reuse saved assignment CSVs and only replot",
    )
    args = parser.parse_args()
    generate_blue_hydrogen_gate_maps(reuse_assignments=args.reuse_assignments)
