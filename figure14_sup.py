...
from seismic_risk import srf_listed_colormap
...
def plot_china_srf_with_provinces(
    ...
    left = 0.5 - width / 2
    cax = ax.inset_axes([left, -0.12, width, 0.028])
    ...
    cbar.set_label("Seismic Screening Index (SSI)", fontsize=mf["colorbar_label"], labelpad=14)
    cbar.ax.tick_params(labelsize=round(mf["colorbar_tick"] * 0.95, 1), pad=4)
...
plot_china_srf_with_provinces(
    ...
    output_path=chart_path("seismic_risk_srs_map.png"),
)
